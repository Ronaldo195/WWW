...
foreach($targets as $member){
        if(is_numeric($member)){
                ...
                if($valid > 0){
						mysql_query("INSERT INTO ".$site['memberships']." (userid, groupid) VALUES ('".$member."', '".$groupid."')");
						mysql_query("DELETE FROM group_requests WHERE userid = '".$member."' AND groupid = '".$groupid."'");
                        //mysql_query("UPDATE ".$site['memberships']." SET is_pending = '0' WHERE userid = '".$member."' AND groupid = '".$groupid."' LIMIT 1") or die(mysql_error());
                }
        }
}
