...
// Collect variables
$stickie = $input->FilterText($_POST['stickieId']);
	...
	if(is_numeric($stickie)){
		if($linked > 0){
			...
			$sql = mysql_query("SELECT * FROM cms_homes_stickers WHERE userid = '".$user->row['id']."' AND groupid = '-1' AND type = '3' AND id = '".$stickie."' LIMIT 1") or die(mysql_error());
		}
	} else {
		exit;
	}
	...
	if($num > 0){
		if($linked > 0){
			...
			mysql_query("DELETE FROM cms_homes_stickers WHERE userid = '".$user->row['id']."' AND groupid = '-1' AND type = '3' AND id = '".$stickie."' LIMIT 1");
		}
		echo "SUCCESS";
	} else {
		echo "ERROR";
